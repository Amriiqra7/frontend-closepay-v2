    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent sx={{ px: 3, py: 3 }}>
        {loading ? (
          ...
          >
            <Box>
              <Typography sx={{ color: "#64748b", mb: 0.5, fontSize: "0.8125rem" }}>Name</Typography>
              <TextField fullWidth size="small" value={data?.name || "-"} InputProps={{ readOnly: true }} sx={readOnlyInputSx} />
            </Box>
            <Box>
              <Typography sx={{ color: "#64748b", mb: 0.5, fontSize: "0.8125rem" }}>Username</Typography>
              <TextField fullWidth size="small" value={data?.username || "-"} InputProps={{ readOnly: true }} sx={readOnlyInputSx} />
            </Box>
            <Box>
              <Typography sx={{ color: "#64748b", mb: 0.5, fontSize: "0.8125rem" }}>No ID</Typography>
              <TextField fullWidth size="small" value={data?.noId || "-"} InputProps={{ readOnly: true }} sx={readOnlyInputSx} />
            </Box>
            <Box>
            ...
            <Box sx={{ gridColumn: { xs: "1 / -1", md: "1 / -1" } }}>
              <Typography sx={{ color: "#64748b", mb: 0.5, fontSize: "0.8125rem" }}>Roles</Typography>
              <TextField
                fullWidth
                size="small"
...
export default function FnbInternalUserPage() {
  ...
  React.useEffect(() => {
    if (listError) showErrorToast(getApiErrorMessage(listError, "Gagal memuat internal user."));
  }, [listError]);
  ...
  const handleConfirmDelete = React.useCallback(async () => {
    ...
    if (!targetId) return;
    await toastPromise(fnbInternalUser.delete(targetId), {
      loading: `Menghapus user "${target?.name}"...`,
      success: `User "${target?.name}" berhasil dihapus.`,
      error: (error) => getApiErrorMessage(error, "Gagal menghapus user."),
    });
  ...
  const handleOpenDetail = React.useCallback(async (row) => {
    const userId = safeId(row);
    if (!userId) return;
    setDetailDialog({ open: true, row, loading: true, data: null });
    try {
      const response = await fnbInternalUser.getById(userId);
      setDetailDialog({ open: true, row, loading: false, data: response?.data || null });
    } catch (error) {
      setDetailDialog({ open: true, row, loading: false, data: null });
  ...
  const columns = React.useMemo(
    () => [
      ...
      { accessorKey: "name", header: "NAMA", Cell: ({ cell }) => <Typography variant="body2">{cell.getValue()}</Typography> },
      { accessorKey: "username", header: "USERNAME", Cell: ({ cell }) => <Typography variant="body2">{cell.getValue()}</Typography> },
      { accessorKey: "noId", header: "NO ID", Cell: ({ cell }) => <Typography variant="body2">{cell.getValue()}</Typography> },
      {
        accessorKey: "roles",
        header: "ROLES",
        enableSorting: false,
        Cell: ({ cell }) => <Typography variant="body2">{(cell.getValue() || []).join(", ") || "-"}</Typography>,
      },
      {
        ...
        Cell: ({ cell }) => (
          <Chip
            ...
            size="small"
            sx={{
              bgcolor: cell.getValue() ? "#d1fae5" : "#fee2e2",
              color: cell.getValue() ? "#065f46" : "#991b1b",
              fontWeight: 700,
              fontSize: "0.75rem",
              height: 24,
              border: "none",
    ...
    renderRowActions: ({ row }) => (
      <Box display="flex" gap={0.5}>
        <Tooltip title="Detail" arrow>
          <IconButton size="small" onClick={() => handleOpenDetail(row.original)} sx={{ color: "#1976d2" }}>
        ...
        <Tooltip title="Hapus" arrow>
          <IconButton size="small" onClick={() => setDeleteDialog({ open: true, row: row.original })} sx={{ color: "#d32f2f" }}>
  ...
  return (
    <Box sx={pageContainerSx}>
      ...
      <InternalUserDetailDialog
        open={detailDialog.open}
        onClose={() => setDetailDialog({ open: false, row: null, loading: false, data: null })}
