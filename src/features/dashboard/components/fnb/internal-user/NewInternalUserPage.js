...
export default function NewInternalUserPage() {
  ...
  const handleSubmit = async () => {
    if (!form.name.trim() || !form.username.trim() || !form.noId.trim() || !form.password.trim() || form.roles.length === 0) return;
    setIsSubmitting(true);
    try {
      await toastPromise(
        fnbInternalUser.create({
          name: form.name.trim(),
        ...
        }),
        {
          ...
          success: "Internal user berhasil dibuat.",
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: "1fr", gap: 2.5 }}>
      <Paper elevation={0} sx={{ borderRadius: 3, border: "1px solid #e5e7eb", p: { xs: 2, md: 3 } }}>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2 }}>
          ...
          <Box>
            <Typography sx={labelSx}>Password{reqMark}</Typography>
            <TextField
              fullWidth
              size="small"
              required
        ...
        <Stack direction="row" justifyContent="flex-end" spacing={1.25} sx={{ mt: 3 }}>
          ...
          <Button
            ...
            variant="contained"
            disabled={isSubmitting || !form.name.trim() || !form.username.trim() || !form.noId.trim() || !form.password.trim() || form.password.length < 8 || form.roles.length === 0}
