...
// Mock data untuk Data Log PPOB
const generateMockLogs = () => {
  const logs = [];
  const statuses = ['Berhasil', 'Gagal', 'Pending'];
  const jenisLayanan = ['PLN', 'PDAM', 'Telkom', 'Indihome', 'BPJS'];
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    logs.push({
      id: i,
      tanggal: date,
      noPelanggan: `1234567890${i}`,
      jenisLayanan: jenisLayanan[i % jenisLayanan.length],
      nominal: 200000 + (i * 20000),
      status: statuses[i % statuses.length],
      keterangan: `Pembayaran ${jenisLayanan[i % jenisLayanan.length]} ${i}`,
    });
  }
...
export default function DataLogPPOBList() {
  ...
  const [searchNoPelanggan, setSearchNoPelanggan] = useState('');
  ...
  const handleJenisLayananFilterChange = useCallback((event) => {
    setJenisLayananFilter(event.target.value);
  }, []);

  const handleSearchNoPelangganChange = useCallback((event) => {
    setSearchNoPelanggan(event.target.value);
  }, []);
  ...
  const handleResetFilter = useCallback(() => {
    ...
    setSearchNoPelanggan('');
    setColumnFilters([]);
  }, []);

  const hasActiveFilters = useMemo(
    () => dateRange.start || dateRange.end || statusFilter || jenisLayananFilter || searchNoPelanggan,
    [dateRange, statusFilter, jenisLayananFilter, searchNoPelanggan]
  );
  ...
  const filteredData = useMemo(() => {
    ...
    if (jenisLayananFilter) {
      filtered = filtered.filter((item) => item.jenisLayanan === jenisLayananFilter);
    }

    if (searchNoPelanggan) {
      filtered = filtered.filter((item) =>
        item.noPelanggan.toLowerCase().includes(searchNoPelanggan.toLowerCase())
      );
    }
    ...
    return filtered;
  }, [dateRange, statusFilter, jenisLayananFilter, searchNoPelanggan, columnFilters]);
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6} md={2}>
              <TextField
                ...
                value={searchNoPelanggan}
                onChange={handleSearchNoPelangganChange}
                size="small"
                sx={{
            ...
            <Grid item xs={12} sm={6} md={2}>
              <TextField
                ...
                onChange={handleJenisLayananFilterChange}
                size="small"
                SelectProps={{
                  displayEmpty: true,
                  renderValue: (selected) => selected || 'Semua Layanan',
                }}
                sx={{
                  '& .MuiOutlinedInput-root': {
                    '&:hover fieldset': {
              ...
              >
                ...
                <MenuItem value="Indihome">Indihome</MenuItem>
