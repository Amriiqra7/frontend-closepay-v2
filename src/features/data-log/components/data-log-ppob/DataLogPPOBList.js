...
import DataLogPPOBDetailDialog from './DataLogPPOBDetailDialog';
...
// Mock data untuk tipe transaksi
const mockTipeTransaksi = [
  { id: '1', name: 'PLN' },
  { id: '2', name: 'PDAM' },
  { id: '3', name: 'Telkom' },
  { id: '4', name: 'Indihome' },
...
const generateMockLogs = () => {
  const logs = [];
  const events = ['PPOB PAYMENT', 'PPOB INQUIRY', 'PPOB STATUS'];
  const statuses = ['Berhasil', 'Gagal'];
  const tipeTransaksi = ['PLN', 'PDAM', 'Telkom', 'Indihome', 'BPJS'];
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    logs.push({
      ...
      tipeTransaksi: tipeTransaksi[i % tipeTransaksi.length],
      ...
      responseDetail: {
        ...
        error: i % 3 === 0 ? {
          ...
          params: '',
          type: 'FAILED_PPOB',
          message: `PPOB payment failed [Code: da22b${i}]`,
          data: '',
          paging: '',
        } : null,
        event: events[i % events.length],
...
export default function DataLogPPOBList() {
  ...
  const [selectedTipeTransaksi, setSelectedTipeTransaksi] = useState(null);
  ...
  const handleResetFilter = useCallback(() => {
    ...
    setSelectedTipeTransaksi(null);
    setColumnFilters([]);
  }, []);

  const hasActiveFilters = useMemo(
    () => selectedCompany !== null || selectedTipeTransaksi !== null,
    [selectedCompany, selectedTipeTransaksi]
  );

  // Filter data berdasarkan company dan tipe transaksi
  const filteredData = useMemo(() => {
    ...
    if (selectedTipeTransaksi) {
      filtered = filtered.filter((item) => item.tipeTransaksi === selectedTipeTransaksi.name);
    }
    ...
    return filtered;
  }, [selectedCompany, selectedTipeTransaksi, columnFilters]);
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            />
            <Autocomplete
              options={mockTipeTransaksi}
              getOptionLabel={(option) => option.name}
              value={selectedTipeTransaksi}
              onChange={(event, newValue) => setSelectedTipeTransaksi(newValue)}
              sx={{ minWidth: 300 }}
              renderInput={(params) => (
                <TextField
                  {...params}
