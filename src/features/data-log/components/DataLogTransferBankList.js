...
// Mock data untuk Data Log Transfer Bank
const generateMockLogs = () => {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    logs.push({
      id: i,
      tanggal: date,
      rekeningAsal: `1234567890${i}`,
      rekeningTujuan: `9876543210${i}`,
      nominal: 500000 + (i * 50000),
      bank: banks[i % banks.length],
      status: statuses[i % statuses.length],
      keterangan: `Transfer Bank ${i}`,
    });
  }
...
export default function DataLogTransferBankList() {
  ...
  const handleSearchRekeningChange = useCallback((event) => {
    setSearchRekening(event.target.value);
  }, []);
  ...
  const hasActiveFilters = useMemo(
    () => dateRange.start || dateRange.end || statusFilter || searchRekening,
    [dateRange, statusFilter, searchRekening]
  );
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchRekening) {
      filtered = filtered.filter((item) =>
        item.rekeningAsal.toLowerCase().includes(searchRekening.toLowerCase()) ||
        item.rekeningTujuan.toLowerCase().includes(searchRekening.toLowerCase())
      );
    }
    ...
    return filtered;
  }, [dateRange, statusFilter, searchRekening, columnFilters]);
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <TextField
                ...
                onChange={handleSearchRekeningChange}
                size="small"
                sx={{
