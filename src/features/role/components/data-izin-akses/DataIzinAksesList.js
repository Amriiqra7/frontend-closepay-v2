...
export default function DataIzinAksesList() {
  ...
  const [detailDialog, setDetailDialog] = useState({ open: false, dataIzinAkses: null });
  ...
  const handleEdit = useCallback((item) => {
    router.push(`${pathname}/${item.id}/edit`);
  }, [router, pathname]);
  ...
  return (
    <>
      ...
      {/* Detail Dialog */}
      <DataIzinAksesDetailDialog
        open={detailDialog.open}
        onClose={() => setDetailDialog({ open: false, dataIzinAkses: null })}
        dataIzinAkses={detailDialog.dataIzinAkses}
      />
